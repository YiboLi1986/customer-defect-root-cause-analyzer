...
class BuildCaseJsonV2WithNotes:
    """
    V2: Same as BuildCaseJson, plus a 4th sheet "Notes" that contains:
      - WorkItemId
      - AdditionalNotes (string, may include "==== State: ... ====" blocks)
    ...
    Notes:
      - additional_notes is kept as list[str] to be future-proof (in case multiple rows exist per WorkItemId).
      - We do NOT parse state blocks here; we keep raw text for downstream parsing/filtering.
    """
    ...
    def __init__(
        ...
        output_dir: str,
        output_name: str = "cases_v2.jsonl",
        ...
        notes_sheet: Union[str, int] = "Notes",  # NEW
    ...
    def _aggregate_additional_notes(self, df_notes: pd.DataFrame) -> pd.DataFrame:
        """
        NEW: Aggregate AdditionalNotes per WorkItemId into list[str].
        """
    ...
    def _iter_records(self, df_out: pd.DataFrame, keep_fields: List[str]) -> Iterable[Dict[str, Any]]:
        for _, row in df_out.iterrows():
            ...
            yield {
                ...
                "additional_notes": row.get("additional_notes", []),  # NEW
...
if __name__ == "__main__":
    ...
    builder = BuildCaseJsonV2WithNotes(
        xlsx_path=xlsx_path,
        keep_fields=KEEP_FIELDS,
        output_dir="backend/src/output",
        output_name="cases_v2.jsonl",
        output_format="jsonl",
    ...
    print(f"[BuildCaseJsonV2WithNotes] Wrote: {out_path}")
