...
class RootCauseLLMAnnotatorV3StateAware:
    """
    V3 (RootCause only) Strategy:

    - Input: cases_v3.jsonl (built by BuildCaseJsonV3WithNotes), each record retains full original fields:
        {
          "work_item_id": ...,
          "fields": {..., includes Product Name/Version + SCM + CodeChange + Code Change/Non-code Change},
          "comment": [...],
          "repro_steps": [...],
    ...
    - LLM input (only when evidence exists):
        We ONLY feed:
          - fields
          - repro_steps (trimmed)
          - evidence.notes_used (ONLY state == Assigned or Investigating)

        We DO NOT feed comments to reduce noise.

    - Decision:
        A) If Assigned/Investigating evidence exists -> call LLM
        B) If no such evidence -> skip LLM (keep existing root_cause if present; otherwise set to "")

    - Output: copy of each record with only "root_cause" filled, others unchanged,
    ...
    def __init__(
        ...
        max_state_blocks: int = 8,
        max_state_block_chars: int = 2500,
        ...
        # V3: hint control (does not change the prompt files; it only enriches evidence.policy)
        include_code_change_hint: bool = True,
    ) -> None:
        ...
        self.include_code_change_hint = bool(include_code_change_hint)
    ...
    def _prepare_case_for_llm(self, case: Dict[str, Any], evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # V3: optional hint based on CodeChange flag (keeps responsibility boundaries clean)
        if self.include_code_change_hint:
            code_change_flag = self._safe_get_code_change_flag(fields)
            if code_change_flag is False:
                policy += " Metadata indicates Non-Code Change (no associated code changelist)."
            elif code_change_flag is True:
                policy += " Metadata indicates Code Change exists (SCM/changelist may be present)."
    ...
    @staticmethod
    def _safe_get_code_change_flag(fields: Dict[str, Any]) -> Optional[bool]:
        """
        V3 cases store CodeChange inside fields. We accept bool or "true/false" strings.
        """
        if not isinstance(fields, dict):
            return None
        v = fields.get("CodeChange", None)
        if v is None:
            return None
    ...
    def _parse_llm_json(text: str) -> Dict[str, Any]:
        ...
        # Enforce single key output (optional but safer for reviewers)
...
if __name__ == "__main__":
    INPUT_JSONL = "backend/src/output/cases_v3.jsonl"
    OUTPUT_JSONL = "backend/src/output/cases_v3_with_root_cause.jsonl"

    # You can keep using the V2 prompts for now
    SYSTEM_PROMPT_PATH = "backend/src/prompt/root_cause_only_v3.system.txt"
    USER_PROMPT_PATH = "backend/src/prompt/root_cause_only_v3.user.txt"

    annotator = RootCauseLLMAnnotatorV3StateAware(
        input_jsonl=INPUT_JSONL,
        output_jsonl=OUTPUT_JSONL,
        system_prompt_path=SYSTEM_PROMPT_PATH,
        user_prompt_path=USER_PROMPT_PATH,
        temperature=0.2,
        limit=30,  # set int for quick test
        max_state_blocks=8,
        max_state_block_chars=2500,
        max_repro_steps=5,
        max_repro_chars=1200,
        attach_evidence_debug=True,
        include_code_change_hint=True,
    )

    out_path = annotator.run()
    print(f"[RootCauseLLMAnnotatorV3StateAware] Wrote: {out_path}")
